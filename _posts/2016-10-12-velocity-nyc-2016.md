---
layout: post
title: "Retour sur la Velocity New York Conférence 2016"
...
Nous étions cette année à New York, à quelques blocs de Time Square, pour suivre l’édition New Yorkaise de la Vélocity Conférence 2016.
...
C’est l’occasion de suivre une conf de très haute qualité avec entre 4 et 5 tracks parallèles de conférences dédiées aux problématiques de performances et de scalabilité.
On remarque que d’année en année la conférence s’est réorienté autour du mouvement devOps, alors qu’elle était précédemment beaucoup plus centrée sur la webPerf (desktop et mobile).

La conférence commence par l’Ignite (sorte de mini conférence dans la conférence), basée sur un format court (type Lightning Talk) de 5 minutes pour une présentation de 5 slides défilant automatiquement. On retiendra de cette première partie des conférences intéressantes partageant les tristes chiffres de la diversité dans la tech aux US, mais aussi une conférence très drôle de [@beldhalpern](https://twitter.com/bendhalpern) de [@ThePracticalDev](https://twitter.com/ThePracticalDev) sur des parodies des livres OReilly (voir le [O RLY Cover Generator](https://dev.to/rly)) :
...
L’ignite c’est fini sur le célèbre Ignite Karaoké où 16 volontaires se sont prêtés au jeu de cet exercice hilarant mais tellement difficile, consistant à improviser une conférence sur le sujet de son choix sur 5 slides inconnues de l’orateur et qui défile automatiquement au bout de quelques secondes 😃. Ce qu’on fais aussi chez M6Web de temps en temps nommé [Karaoké Slideshow](http://tech.m6web.fr/organiser-des-conferences-technique-en-interne) et que Kenny avait animé lors d’un Forum PHP ([voir la vidéo](https://www.youtube.com/watch?v=Ln12meWM1pE)).
...
Cela consiste généralement en un bot ou une IA posé sur un outil de chat type Slack, Flowdock ou Hipchat, permettant de simplifier la communication entre différentes équipes et les différents outils (ticketing, alerting, monitoring, état d’une machine, etc). Une démo de l’IA de Dynatrace à reconnaissance vocale à été faite, montrant comment par la voie, on pouvait recevoir dans l’outil de Chat les infos sur les incidents de la veille, créer les tickets de support etc. [Voir ici](http://dynatrace.com/meet-davis). Un peu gadget, mais rigolo.
...
Plusieurs conférences avaient pour but de comparer ce que l’on pouvait obtenir de nos jours via du WebMobile vs ce que l’on a sur les apps natives. Le fossé c’est énormément rétréci et les WebApps ont désormais accès à la plupart des fonctionnalités présentes côté natif :
...
C’est ce qui nous amène aux Progressive Web Apps : PWA
...
Arriveront prochainement sur AMP, le support des formulaires, des optimisations avancées d’images via le Google AMP Cache, des services Workers pour AMP pour ne jamais téléchargé AMP dans le « chemin critique » du chargement de la page.
...
Nous avons aussi vu une conférence sur l’optimisation de la consommation des webApps en terme de CPU / temps de réponses, notamment via l’étude des capacités JS de chacun des devices/OS avec le benchmark JetStream Javascript.

On découvre notamment que l’iPhone7 a des capacités assez impressionnantes, contrairement aux iPhone 5C, que le mode « économie d’énergie » ou encore qu’après une bonne insolation, les devices sont beaucoup moins performants. D’excellentes slides a voir ici : [hearne.me/2hot](http://hearne.me/2hot)
...
Tammy Everts ([@tameverts](https://twitter.com/tameverts) de Soasta) et Pat Meenan ([@patmeenan](https://twitter.com/patmeenan?lang=fr de Google et créateur de WebPageTest)) nous ont fait un gros retour basé sur toutes les métriques récoltées par Soasta mPulse (outils de Real User Monitoring en SAAS) afin de déterminer des corrélations entre les temps de chargements et d’autres métriques (taux de rebond, conversion etc) grâce à l’application de concept propre au Machine Learning sur une quantité énorme de data. Toujours intéressant.
...
Coté HTTP, on retiendra Hooman Beheshti qui nous a fait un retour d’expérience sur HTTP 2. Après une explication des nouveautés du protocole (binary, single, long-lasting TCP connection, streams encapsulation, frames, bi-directional…), une comparaison avec HTTP 1 nous a été exposée. En conclusion, HTTP2 est complexe et migrer n’est pas juste modifier un paramètre. Bien que cette nouvelle version est légèrement plus rapide, en particulier sur un réseau lent (<1Mbps), le protocole supporte très mal les pertes de paquets ou les fortes congestions à cause de l’unique connexion (TCP slow start). La recommandation est de tester sur chaque site et d’optimiser ses pages selon la version d’HTTP utilisée. Une piste serait HTTP2 over UDP.

Les [slides](http://www.slideshare.net/Fastly/http2-what-no-one-is-telling-you)

# DevOps

De nombreuses conférences avaient pour objectifs d’aborder le bienfaits du DevOps et plus largement les bonnes pratiques liées au mouvement afin de gagner en qualité et fiabilité.

On retiendra notamment la conférence de Cornelia Davis (DevOps: Who does what?) explicitant les différents rôles dans un SDLC (software development life-cycle) et leur répartition en équipes dans l’organisation.
...
## La répartition en équipes proposée :
...
* Ne pas laisser s’accumuler sa dette technique
* Déploiement continu
* Travail en équipes entre Dev / PM / SRE
* Monitoring
* Procédures standards de gestion des incidents
* Post-mortem pour apprendre des ses erreurs…
...
Concernant la sécurité, la conférence de Kelly Lum [@aloria](https://twitter.com/aloria), passait en revu le minimun vital  :
* La sécurité doit être pensée dès la conception
* Permettre aux utilisateurs de reporter facilement des problèmes sécurités et être à l’écoute des réseaux sociaux
...
et voir les vidéos de certaines des conférences [ici](https://www.youtube.com/playlist?list=PL055Epbe6d5Zl1yShG26D2r2TzwDDoDfK)
