...
A l’instar de Treezor, Bedrock possède également de nombreuses Lambda développées en PHP avec Bref. Ces Lambdas sont majoritairement déployées via le [framework serverless](https://www.serverless.com/){:target="_blank"}. PHP n’étant pas nativement supporté par Lambda nous essayons tout de même autant que possible d’utiliser d’autres langages comme Python ou Go, notamment dans le pôle infrastructure.
