...
Chez Bedrock, nous avons aussi rencontré ces problématiques, nous avons réalisé des PoC de différentes solutions (dont SageMaker) et nous avons retenu la plateforme [Databricks](https://databricks.com).
...
A l’instar de Treezor, Bedrock possède également de nombreuses Lambda développées en PHP avec Bref. Ces Lambdas sont majoritairement déployées via le [framework serverless](https://www.serverless.com/). PHP n’étant pas nativement supporté par Lambda et Bref étant principalement maintenu par un seul développeur, nous essayons, autant que possible, d’utiliser d’autres langages comme Python ou Go, notamment dans le pôle infrastructure. 
...
[Code Guru](https://docs.aws.amazon.com/codeguru/latest/reviewer-ug/welcome.html) 
...
Chez Bedrock, les langages utilisés étant principalement Javascript, PHP et Python, Code Guru ne sera pas une solution adéquate dans toutes les situations. Nous avons donc mis en place la solution [KICS](https://kics.io/) qui nous permet, à l’aide de règles [Open Policy Agent](https://www.openpolicyagent.org/), d’effectuer automatiquement de nombreuses validations sur le code infrastructure (Terraform, Docker, YAML, …). KICS est utilisé au travers de [GitHub Actions](https://github.com/features/actions) pour ajouter des commentaires sur les pull requests comme Code Guru est capable de le faire.
