...
Webperf is still a major challenge in today's web industry, influencing user experience, website visibility and overall commercial success. This article summarizes the topics discussed at this year's _We Love Speed_ conference and gives an overview of the 2023 approach to web performance.
...
After pinpointing the performance issues of your website, you can implement many different optimizations to improve its performance. While the list is far from exhaustive, we can go here through some of them in order to get a few ideas.
...
Images, and especially high-quality ones, can take up a lot of a website's loading time, and are often considered the Largest Content of a website, **dictating the LCP value**. However, quite often, a user isn't seeing every image of a website at the same time. That’s where we can use lazy-loading.

The goal of lazy-loading is to make sure that a user is only loading the images they are seeing, and not the ones they aren't displaying yet. Here again, exists an HTML attribute that can take care of everything, named `loading`. It can receive the `lazy` value in order to only load an image when approaching the screen.
...
Nowadays, a lot of websites, us included, are using a lot of third parties. Whether it is in order to monitor what a user is doing so that we can track their issues more precisely, to gather some data for our teams, or simply to gather ads information, we need them.
...
When downloading files from a server (Such as font files, css files, or others), the server does not know what size of packages a user's browser is able to handle. It will thus need to “warm up” first, sending more and more packages at once until the browser has reached its limit, at which point it will adapt and keep sending at the highest possible (and manageable) speed.
