...
Therefore, a big hurdle is that we need to collect the user's consent to be able to connect to our thirdparties API. That's why we need our Consent Manager to be booted and ready. The first improvement to make is to optimize the CMP, to make sure it loads fast and it shares the user’s consent with us as soon as possible.
...
But we know that we still have a lot to do in this area, particularly in terms of long-term follow-up, monitoring and company-wide concern.

Following this conference, there are lots of things we'd like to try out, such as using the `fetchpriority` attribute to improve our LCP, optimising the hydration of our SSR (perhaps with the use of [React Server Components](https://vercel.com/blog/understanding-react-server-components)) or studying the new experimental features.

Finally, on our TV platforms developed in JS, hardware constraints are forcing us to go even further in optimizing performance, particularly in terms of memory usage, and we have a project underway to have a global display with lazy loading. To be continued...
